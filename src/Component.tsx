...
/**
 * Jolly Time Popcorn Japan Brand Page
 * Features:
 * - Image carousel for History section
...
// Utility for image optimization (using wsrv.nl for on-the-fly resizing and WebP conversion)
const optimizeImage = (url: string, width: number = 800) => {
  if (!url) return '';
  if (url.endsWith('.svg')) return url; // Don't optimize SVGs
...
const Navbar = ({ onContactClick }: { onContactClick: () => void }) => {
  ...
  const navItems = [
    { name: 'History', label: 'ストーリー' },
    { name: 'Farmers', label: '家族農家' },
    { name: 'Global', label: '世界の家庭' },
    { name: 'Flavors', label: 'フレーバー' },
    { name: 'About', label: '運営会社' },
  ];
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="relative z-50 block w-24 md:w-32 transition-transform hover:scale-105">
          ...
          {navItems.map((item) => (
            <a 
              ...
              className={cn(
                ...
                isScrolled ? "text-stone-800 hover:text-primary" : "text-white hover:text-yellow-300"
              )}
            >
              {item.label}
            </a>
        ...
        {/* Mobile Menu Button */}
        <button 
          className={cn("md:hidden relative z-50 p-2", isScrolled ? "text-stone-800" : "text-white")}
...
const Hero = ({ onSupportClick }: { onSupportClick: () => void }) => {
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 gap-12 md:items-center">
          ...
          >
            <div className="flex flex-col sm:flex-row items-center gap-6 justify-center md:justify-start">
              ...
              >
                Makuakeで支援する <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-12">
          <div className="w-full md:w-1/2">
             ...
             <p className="text-blue-100 text-lg mb-8 font-medium">
               Jolly Time は、アメリカだけのブランドではありません。世界中の家庭で楽しまれています。
...
const FlavorsSection = () => {
  ...
  const flavors = [
    { 
      ...
      color: "bg-yellow-400",
      tag: "THEATER STYLE",
      // Popped corn with box image (main display)
              ...
              >
                <div className={cn(
                  ...
                  item.color
                   ...
                   {/* Main Product Image - Popped Corn (default) */}
                   <motion.div 
                   ...
                   {/* Quick Add Button */}
                   <button className="absolute bottom-5 right-5 w-12 h-12 bg-white text-stone-900 rounded-full flex items-center justify-center shadow-lg translate-y-20 group-hover:translate-y-0 transition-transform duration-300 hover:bg-stone-900 hover:text-white z-30">
                      <ArrowRight size={20} />
                   </button>
                </div>
...
const MovieNightSection = ({ onSupportClick }: { onSupportClick: () => void }) => {
  return (
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          <div className="lg:col-span-2">
            <div className="bg-white p-4 rounded-xl inline-block mb-8 w-64">
               <img 
          ...
          <div>
            ...
            <div className="text-stone-300 space-y-2">
               ...
               <p className="flex items-start gap-2 text-sm mt-2">
                 ...
                 <span>東京都台東区小島2-10-7</span>
               </p>
               <p className="text-sm mt-1 px-6">050-1807-2888</p>
            </div>
          </div>
...
export function JollyTimeLineupVariant() {
  ...
  const openContactForm = () => {
    if (window.Tally) {
      ...
      // Fallback if script hasn't loaded or fails
  ...
  return (
    <div className="font-sans antialiased text-stone-900 bg-stone-50 selection:bg-yellow-400 selection:text-stone-900">
      ...
      <Hero onSupportClick={() => setShowComingSoon(true)} />
      <HistorySection />
      <FarmersSection />
      <GlobalSection />
      <FlavorsSection />
      <MovieNightSection onSupportClick={() => setShowComingSoon(true)} />
        ...
        {showComingSoon && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="relative bg-white rounded-3xl p-12 max-w-md w-full shadow-2xl text-center overflow-hidden"
            >
              {/* Decorative Background for Modal */}
              <div className="absolute top-0 left-0 w-full h-2 bg-yellow-400" />
              <div className="absolute -top-12 -right-12 w-24 h-24 bg-red-100 rounded-full blur-2xl" />
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-3xl font-black text-stone-900 mb-4">Coming Soon!</h3>
                <p className="text-stone-600 font-medium mb-8 leading-relaxed">
                  Jolly Time Popcorn の Makuake プロジェクトは現在準備中です。公開まで楽しみにお待ちください。
                </p>
                <button 
                  onClick={() => setShowComingSoon(false)}
                  className="w-full bg-primary text-white py-4 rounded-full font-black text-xl hover:bg-red-700 transition-all shadow-lg shadow-red-200"
                >
                  閉じる
                </button>
              </div>
